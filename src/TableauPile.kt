class TableauPile(var cards: MutableList<Card> = mutableListOf()) {
    ...
    private fun suitCheck(card1: Card, card2: Card): Boolean {
        if ((redSuits.contains(card1.suit) && blackSuits.contains(card2.suit)) || (blackSuits.contains(card1.suit) && redSuits.contains(card2.suit))){
            return true
